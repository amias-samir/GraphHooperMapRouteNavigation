...
class RouteNavigationRouteController extends GetxController {
  ...
  updateDistanceBtnCOOrds({required double distance}) {
    ...
    debugPrint(
        'RouteNavigationRouteController calculateDistance :  ${distanceBtnCOOrds.value}');
  }
  ...
  findInstructionsCoordsAndIndex(
    ...
    InstructionsCoordsAndIndexList instructionsCoordsAndIndexList =
        await compute(
            computingInstructionsCoordsAndIndex, directionRouteResponse);
  ...
  Future<InstructionsCoordsAndIndexList> computingInstructionsCoordsAndIndex(
      DirectionRouteResponse directionRouteResponse) async {
    List<Instructions> instructions =
        directionRouteResponse.paths![0].instructions!;
    ...
    if (instructions.isNotEmpty) {
      for (int index = 0; index < instructions.length; index++) {
        ...
        instructionsCoordList.add(directionRouteResponse
            .paths![0].points!.coordinates![instruction.interval![0]]);
      }
    }
